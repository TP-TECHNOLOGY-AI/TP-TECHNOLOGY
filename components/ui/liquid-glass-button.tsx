...
const liquidbuttonVariants = cva(
  "inline-flex items-center transition-colors justify-center cursor-pointer gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 shrink-0 [&_svg]:shrink-0 outline-none",
  {
    variants: {
      variant: {
...
const colorVariants: Record<ColorVariant, { outer: string; inner: string; button: string; textColor: string; textShadow: string }> = {
  ...
  primary: {
    outer: "bg-gradient-to-b from-[#0d2235] to-[#4a90a4]",
    inner: "bg-gradient-to-b from-[#1a3a52] via-[#2d5f7a] to-[#4a90a4]",
    button: "bg-gradient-to-b from-[#4a90a4] to-[#1a3a52]",
    textColor: "text-white",
    textShadow: "[text-shadow:_0_-1px_0_rgb(26_58_82_/_100%)]",
  },
  success: {
    ...
    return (
      ...
      >
        <div
          ...
          style={{ transition: transitionStyle, filter: isHovered && !isPressed && !isTouchDevice ? "brightness(1.05)" : "none" }}
        />
        <button
          ref={ref}
          className={cn(
